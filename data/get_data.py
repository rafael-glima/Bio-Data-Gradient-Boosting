...
def get_data(file_path):
    ...
    for row in worksheet.iter_rows():
        ...
        # referAllele = str(row[7].value)
        # if referAllele in dict_ReferAllele:    #TUMOR_ALLELE

        #     Row.append(dict_ReferAllele[referAllele])
        # else:
        #     nRefer = len(dict_ReferAllele)
        #     dict_ReferAllele[referAllele] = nRefer + 1
        #     Row.append(dict_ReferAllele[referAllele])

        #TUMOR_ALLELE
        tumorAlle = row[8].value
        Row.append(transferRef(tumorAlle))
        tumorAlle = str(row[8].value)
        # if tumorAllele in dict_TumorAllele:    #TUMOR_ALLELE

        #     Row.append(dict_TumorAllele[tumorAllele])
        # else:
        #     nTumorAl = len(dict_TumorAllele)
        #     dict_TumorAllele[tumorAllele] = nTumorAl + 1
        #     Row.append(dict_TumorAllele[tumorAllele])
