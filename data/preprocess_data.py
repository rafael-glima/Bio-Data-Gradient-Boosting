...
NUM_TRAIN = 300000 #330000
NUM_VALID = 1 #31000 #1000
...
def preprocess_data(file_path):
    ...
    def createFeatures(data):
        ...
        Data = data[:, 0:data.shape[1]-2] #data[:, 0:data.shape[1]-2]
        ...
        print(Tumor_Allele[:100])
        print(Reference_Allele[:100])

        y = keras.utils.to_categorical(np.array(Data[:, 0]).astype(int))
        x = np.reshape(Data[:, 1:data.shape[1]-2], (-1, 6, 1)) #np.reshape(Data[:, 1:data.shape[1]-2], (-1, 6, 1))
